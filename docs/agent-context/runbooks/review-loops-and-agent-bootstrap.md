...
nonexistent surfaces with identical confidence. (Promoted 2026-08-07 by
owner direction from the 2026-08-06 lessons entries — single-repository
citation, corroborated in-hub the same day when this check caught ten
citation defects in the backport plan that carried it.)
...
- **Principle-Level Diagnosis:** when findings are not independent
  ...
  produced it; the principle dictates the remediation's shape (a §12
  violation gets a firing gate, not another prose rule).
