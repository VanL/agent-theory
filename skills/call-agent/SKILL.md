...
rounds, via this skill's own documented invocation; see
`docs/plans/2026-07-14-call-agent-skill-plan.md` §7).

## Purpose

Invoke a second agent family for independent review ([DOM-11]) using
...
- The [DOM-13] probe refresh of the agent inventory.
...
- Updates `docs/implementation/03-agent-inventory.md` after probes
...
### 6. Probe procedure ([DOM-13] inventory refresh)
...
`docs/implementation/03-agent-inventory.md`: verified usable /
...
- If one agent family dominates all reviews for convenience, that is
  [DOM-13] drift — the inventory's review-preference note, not habit,
